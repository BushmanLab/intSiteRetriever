...
test_that("should return df with 2 columns", {
    expect_is(is_exist, "data.frame")
    expect_named(is_exist, c("sampleName", "refGenome"), ignore.order=TRUE)
})

test_that("ref genomes are correct or NA", {
    is_exist <- (is_exist)$refGenome
